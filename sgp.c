...
const double MASS_OF_EARTH = 5.972E24;
const double XKMPER = 6378.135;
const double J_3 = -.253881E-5;

/* TODO: Own header */
static vec vec_mul(double scalar, vec vector) {
    vec result = { vector.x * scalar, vector.y * scalar, vector.z * scalar };
    return result;
}

static vec vec_add(vec vector1, vec vector2) {
...
sgp_result sgp4(tle_data *data, long minutes_since_epoch) {
    ...
    double a_E = 1.0; /* TODO: Const */
    double S = 1.01222928;
    double Q_0 = 1.88027916E-9;

    double k_e = sqrt(G * MASS_OF_EARTH);
    double k_2 = 5.413080E-4;
    double k_4 = .62098875E-6;
    double A_3_COMMA_0 = -J_3 * a_E * a_E * a_E; /* -J_3 * cube(earth_radius(0)) */
    ...
    double C_2 =
            q_0_minus_s_pow_4 * pow(xi, 4) * d_d_n_0 * pow(1 - square(eta), -7.0 / 2.0) *
            ((d_d_a_0 * (1 + THREE_HALVES * square(eta) + 4 * data->eccentricity * eta + data->eccentricity * cube(eta))) +
             ...
             (8 + 24 * square(eta) + 3 * pow(eta, 4)));
    double C_1 = data->drag * C_2;
    double C_3 = (q_0_minus_s_pow_4 * pow(xi, 5) * A_3_COMMA_0 * d_d_n_0 * a_E * sin(i_0)) /
                 (k_2 / data->eccentricity);
    double C_4 = 2 * d_d_n_0 * q_0_minus_s_pow_4 * pow(xi, 4) * d_d_a_0 * square(beta_0) * pow(1 - square(eta), -7.0 / 2) *
                 ((2 * eta * (1 + data->eccentricity * eta) + ONE_HALF * data->eccentricity + ONE_HALF * cube(eta)) -
                  ((2 * k_2 * xi) / d_d_a_0 * (1 - square(eta))) *
                  (
                          3 * (1 - 3 * square(theta)) *
                          (1 + THREE_HALVES * square(eta) - 2 * data->eccentricity - ONE_HALF * data->eccentricity * cube(eta)) +
                          (3.0 / 4.0) * (1 - square(theta) *
                          (2 * square(eta) - data->eccentricity * eta * data->eccentricity * cube(eta)) *
                          cos(2 * omega_0))
                  ));
    double C_5 = 2 * q_0_minus_s_pow_4 * pow(xi, 4) * d_d_a_0 * square(beta_0) *
    ...
    double D_4 = TWO_THIRDS * d_d_a_0 * cube(xi) * (221 * d_d_a_0 + 31 * s) * pow(C_1, 4);
    double M_DF = M_0 +
            (1 + ((3 * k_2 * (-1 + 3 * square(theta))) / (2 * square(d_d_a_0) * cube(beta_0))) +
             ((3 * square(k_2) * (13 - 78 * square(theta) + 137 * pow(theta, 4))) /
              (16 * pow(d_d_a_0, 4) * pow(beta_0, 7)))) *
            d_d_n_0 * minutes_since_epoch;
    double omega_DF = omega_0 +
            ((-(3 * k_2 * (1 - 5 * square(theta))) / (2 * square(d_d_a_0) * pow(beta_0, 3))) +
             ((3 * square(k_2) * (7 - 114 * square(theta) + 395 * pow(theta, 4))) / (16 * pow(d_d_a_0, 4) * pow(beta_0, 8))) +
             ((5 * k_4 * (3 - 36 * square(theta) * 49 * pow(theta, 4))) / (4 * pow(d_d_a_0, 4) * pow(beta_0, 8)))) *
            d_d_n_0 * minutes_since_epoch;
    double OMEGA_DF = OMEGA_0 +
            ((-(3 * k_2 * theta) / (square(d_d_a_0) * pow(beta_0, 4))) +
             ((3 * square(k_2) * (4 * theta - 19 * cube(theta))) / (2 * pow(d_d_a_0, 4) * pow(beta_0, 8))) +
             ((5 * k_4 * theta * (3 - 7 * square(theta))) / (2 * pow(d_d_a_0, 4) * pow(beta_0, 8)))) *
            d_d_n_0 * minutes_since_epoch;
    double delta_omega = data->drag * C_3 * cos(omega_0) * minutes_since_epoch;
    double delta_M = -TWO_THIRDS * q_0_minus_s_pow_4 * data->drag * pow(xi, 4) *
            (a_E / (data->eccentricity * eta)) *
            (pow(1 + eta * cos(M_DF), 3) - pow(1 + eta * cos(M_0), 3));
    double M_p = M_DF + delta_omega + delta_M;
    double omega = omega_DF - delta_omega - delta_M;
    double OMEGA = OMEGA_DF -
            (21.0 / 2.0) * ((d_d_n_0 * k_2 * theta) / (square(d_d_a_0) * square(beta_0))) *
            C_1 * square(minutes_since_epoch);
    double e = data->eccentricity - data->drag * C_4 * minutes_since_epoch -
            data->drag * C_5 * (sin(M_p) - sin(M_0));
    double a = d_d_a_0 *
            square(1 - C_1 * minutes_since_epoch -
                   D_2 * square(minutes_since_epoch) -
                   D_3 * cube(minutes_since_epoch) -
                   D_4 * pow(minutes_since_epoch, 4));
    double L = M_p + omega + OMEGA + d_d_n_0 * (THREE_HALVES * C_1 * square(minutes_since_epoch) +
            (D_2 + 2 * square(C_1)) * cube(minutes_since_epoch) +
            (1.0 / 4.0) * (3 * D_3 + 12 * C_1 * D_2 + 10 * cube(C_1) * pow(minutes_since_epoch, 4)) +
            ((1.0 / 5.0) * (3 * D_4 + 12 * C_1 * D_3 + 6 * square(D_2) + 30 * square(C_1) * D_2 + 15 * pow(C_1, 4) * pow(minutes_since_epoch, 5))));
    double beta = sqrt(1 - square(e));
    double n = k_e / sqrt(cube(a));
    /* TODO: Drop terms perigee < 220 */
    ...
    double AYN_COS;
    double AYN_SIN;
    double U = fmod(L_T - OMEGA, 2 * 3.14159265);
    double E_w_i = U;
    for (int i = 1; i <= 10; ++i) {
